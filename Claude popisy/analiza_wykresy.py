...
class Report:
    ...
    # Ekstrema liczone w FAZIE MIOTANIA (do chwili zwolnienia). Po zwolnieniu
    # pocisk spada swobodnie i rozpędza się grawitacyjnie, więc tamte maksima
    # nic nie mówią o maszynie - są raportowane osobno.
    peakSpeed: float = 0.0
    peakSpeedTime: float = 0.0
    peakEk: float = 0.0
    peakEkArmDeg: float = 0.0      # przy jakim kącie ramienia Ek jest maks.
    peakOmega: float = 0.0
    freeFallPeakSpeed: float = 0.0  # maks. prędkość w locie swobodnym

    maxHeight: float = 0.0
    landingX: Optional[float] = None
    landingIndex: Optional[int] = None
...
def analyze(cols: Dict[str, List[float]], name: str = "",
    ...
    # --- ekstrema --------------------------------------------------------
    # Okno "fazy miotania": od startu do zwolnienia. To w nim szukamy maksimów,
    # bo po zwolnieniu pocisk tylko spada i rozpędza się od grawitacji -
    # jego prędkość tuż przed uderzeniem w ziemię nic nie mówi o maszynie.
    last = iRel if iRel is not None else len(t) - 1
    window = range(0, last + 1)

    iFastW = max(window, key=lambda i: speed[i])
    r.peakSpeed, r.peakSpeedTime = speed[iFastW], t[iFastW]

    iEk = max(window, key=lambda i: ek[i])
    r.peakEk = ek[iEk]
    r.peakEkArmDeg = math.degrees(cols["armAngle"][iEk])

    if "armOmega" in cols:
        r.peakOmega = max((cols["armOmega"][i] for i in window), key=abs)

    r.maxHeight = max(y)
    if iRel is not None and iRel < len(speed) - 1:
        r.freeFallPeakSpeed = max(speed[iRel:])
    return r
...
def printReport(r: Report) -> None:
    ...
    print("-" * 66)
    print(" FAZA MIOTANIA (do zwolnienia):")
    ...
    print(f"   maks. prędkość kątowa ramienia: {r.peakOmega:9.2f} rad/s")
    print(" LOT SWOBODNY:")
    print(f"   maks. wysokość               : {r.maxHeight:10.2f} m")
    if r.freeFallPeakSpeed:
        print(f"   prędkość maks. w locie       : {r.freeFallPeakSpeed:10.2f} m/s"
              f"   (rozpędzona grawitacją, nie przez maszynę)")

    # NAJCIEKAWSZA OBSERWACJA: czy zwolniono w najlepszym momencie.
    # Porównujemy energię w chwili zwolnienia z maksimum osiągniętym
    # w fazie miotania - jeśli maksimum było wcześniej, spust jest spóźniony.
...
def plot(datasets: List[Dict[str, List[float]]], reports: List[Report],
    ...
    for cols, rep in zip(datasets, reports):
        ...
        # Przycinamy dane do chwili LĄDOWANIA. Po wylądowaniu pocisk sunie
        # po gruncie (w modelu nie ma tarcia), więc dalsze punkty tylko
        # zaśmiecają wykresy poziomymi odcinkami o stałej prędkości.
        end = (rep.landingIndex + 1) if rep.landingIndex is not None \
            else len(cols["t"])
        cols = {k: v[:end] for k, v in cols.items()}
        t = cols["t"]

        ax[0][0].plot(t, cols["ekBody"], label=label)
        # wykres optymalizacyjny rysujemy TYLKO dla fazy miotania - po
        # zwolnieniu kąt ramienia i energia pocisku nie mają już związku
        iRel = next((i for i, v in enumerate(cols.get("released", []))
                     if v > 0.5), len(t) - 1)
        ax[0][1].plot([math.degrees(a) for a in cols["armAngle"][:iRel + 1]],
                      cols["ekBody"][:iRel + 1], label=label)
...
def main(argv: List[str]) -> int:
    # --- parsowanie argumentów: wyciągamy --save i JEGO wartość, reszta
    #     to ścieżki do plików CSV
    savePath = None
    paths: List[str] = []
    i = 1
    while i < len(argv):
